...
Writes to:
  ...
  - <user-data>/voice-notes.log (rotated at 1 MB, 3 backups)

Level controlled by $VOICE_NOTES_LOG_LEVEL (default: INFO).
...
def configure() -> None:
    ...
    level_name = os.environ.get("VOICE_NOTES_LOG_LEVEL", "INFO").upper()
    ...
    root = logging.getLogger("voice_notes")
    root.setLevel(level)
    ...
    try:
        log_path = _user_data_dir() / "voice-notes.log"
...
def get_logger(name: str) -> logging.Logger:
    return logging.getLogger(f"voice_notes.{name}")
